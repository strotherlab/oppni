...
while ischar(tline) 
        ...
        for krun = 1:length(Input_nifti_file_path)
            ...
            InputStruct(ksub).run(krun).Subject_OutputDirectory           = [InputStruct(ksub).run(1).Output_nifti_file_path '/matfiles'];
            InputStruct(ksub).run(krun).subjectprefix                     = ['_' Output_nifti_file_prefix{1}];
            InputStruct(ksub).run(krun).subjectmask                       = [Output_nifti_file_path{1}  '/masks/' Output_nifti_file_prefix{1} '_mask.nii'];
            
            load(split_info_file{krun});
...
% 
outdir = InputStruct(1).run(1).Subject_OutputDirectory;
l1 = length(outdir);
for is = 2:length(InputStruct)
    l2 = length(InputStruct(is).run(1).Subject_OutputDirectory);
    l1 = length(outdir);
    l = min(l1,l2);
    ind = find(outdir(1:l)~=InputStruct(is).run(1).Subject_OutputDirectory(1:l));
    if isempty(ind) 
        ind(1) = l+1; 
    end
