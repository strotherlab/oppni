...
%%%%%%%%
    ...
    if(  (~strcmpi(InputStruct1(1).run(1).TaskType,'nocontrast') && ~strcmpi(InputStruct2(1).run(1).TaskType,'nocontrast')) && ...
            (~isempty(InputStruct1(1).run(1).TaskType) && ~isempty(InputStruct2(1).run(1).TaskType)) )
            ...
            if( isempty(metric_names) )
                ...
                if( max(diffs_metric(:)) > 1E-6 ) %% any major differences in pipes
                    ...
                    if(max(max(diffs_metric(:,:,1))) >1E-6) 
                        ...
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f, ',metric_names{i},mean(diffs_metric(olix,i,1)) )]; end
                    ...
                    if(max(max(diffs_metric(:,:,2))) >1E-6) 
                        ...
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f, ',metric_names{i},mean(diffs_metric(olix,i,2)) )]; end
                        fprintf(fin,'\t\tMean abs. difference, in runs showing changes: %s\n',cato);
                    end
                    if(max(max(diffs_metric(:,:,3))) >1E-6)
                        ...
                        fprintf(fin,'\tIND (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f, ',metric_names{i},mean(diffs_metric(olix,i,3)) )]; end
                        fprintf(fin,'\t\tMean abs. difference, in runs showing changes: %s\n',cato);
                    end                
                else
...
% %                 end
                ...
                if( prod(Nvox1==Nvox2) ) %% if all matching
                    ...
                    for(ksub=1:Nsubject)
                        ...
                        if( prod(strcmpi(es1,es2)) )
                            ...
                            for(i=1:numel(es1))
                               ...
                               voldif_s(ksub,i) = max(abs(V1.img(:)-V2.img(:)));
                            end
                        else
                            %file mismatch
                    ...
                    if( max(voldif_u(:)) > 1E-6 ) %% any major differences in pipes (subj x ppln x contr)
                        ...
                        fprintf(fin,'\tMean of max. absolute deviations: %f\n',mean( voldif_u(voldif_u>eps) ));
                    else
        ...
        %%% PIPELINE...
                ...
                if( prod(Nvox1==Nvox2) ) %% if all matching
                    ...
                    for(ksub=1:Nsubject)
                        ...
                        fprintf('\nksub %d Nsubject %d', ksub, Nsubject);
                        M1=load_untouch_nii(InputStruct1(ksub).run(1).subjectmask); mask1=double(M1.img);
                        M2=load_untouch_nii(InputStruct1(ksub).run(1).subjectmask); mask2=double(M2.img);
                        ...
                        if( prod(strcmpi(eu1,eu2)) )
                            ...
                            for(i=1:numel(eu1))
                               ...
                               V2=load_untouch_nii([InputStruct2(ksub).run(1).Output_nifti_file_path,'/optimization_results/processed/',eu2{i}]);
                               voldif_u(ksub,i) = max(abs(V1.img(:)-V2.img(:)));
                               totdif_s(ksub,i) = nnz((abs(V1.img(:)-V2.img(:))));
                            end
                        else
                            %file mismatch
                            erstr1=[]; for(i=1:numel(eu1)) erstr1=[erstr1,', ',eu1{i}]; end
                    ...
                    fprintf(fin,'\tMean of max. absolute deviations: %f\n',mean( voldif_u(voldif_u>eps) ));
                    fprintf(fin,'\tMax number of different voxels: %f\n',max( totdif_u(totdif_u>eps) ));
                    else
                    fprintf(fin,'Comparing all pipeline Procs(unnorm)..............................[OK]\n');    
                    end
