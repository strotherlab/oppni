    ...
    if( optcheck1 && optcheck2 )
        ...
        if( Nspm1 ~= Nspm2 )
            ...
            if( isempty(metric_names) )
                ...
                if( max(diffs_ppl) > TOLERANCE ) %% any major differences in pipes
                    
                    fprintf(fin,'Comparing optimal pipelines.......................................[!!]\n');
                    fprintf(fin,'\tDifferences in: ');
                    if(max(abs(diffs_con))     >TOLERANCE) fprintf(fin,'CON, '); end
                    if(max(abs(diffs_fix))     >TOLERANCE) fprintf(fin,'FIX, '); end
                    if(max(max(abs(diffs_ind)))>TOLERANCE) olix = find(max(abs(diffs_ind),[],2) > TOLERANCE );
                ...
                if( max(diffs_metric(:)) > 1E-3 ) %% any major differences in pipes
                    
                    fprintf(fin,'Comparing optimized metrics.......................................[!!]\n');
                    fprintf(fin,'\tDifferences in: \n');
                    if(max(max(diffs_metric(:,:,1))) >1E-3)
                        olix=find( max(diffs_metric(:,:,1),[],2) > 1E-3);
                        fprintf(fin,'\tCON (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        cato=[];
                        for(i=1:length(metric_names)) 
                            cato = [cato,sprintf( '(%s)=%0.3f [%0.3f, %0.3f, %0.3f %0.3f], ', ...
                                metric_names{i},median(diffs_metric(olix,i,1)), ...
                                min(diffs_metric(olix,i,1)),prctile(diffs_metric(olix,i,1),25),prctile(diffs_metric(olix,i,1),75),max(diffs_metric(olix,i,1)) )]; 
                        end
                        fprintf(fin,'\t\tMedian [min, Q1, Q3, max] abs. difference, in runs showing changes: %s\n',cato);
                        
                    end
                    if(max(max(diffs_metric(:,:,2))) >1E-3)
                        olix=find( max(diffs_metric(:,:,2),[],2) > 1E-3);
                        fprintf(fin,'\tFIX (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f [%0.3f, %0.3f, %0.3f %0.3f], ',metric_names{i},median(diffs_metric(olix,i,2)),min(diffs_metric(olix,i,2)),prctile(diffs_metric(olix,i,2),25),prctile(diffs_metric(olix,i,2),75),max(diffs_metric(olix,i,2)) )]; end
                        fprintf(fin,'\t\tMedian [min, Q1, Q3, max] abs. difference, in runs showing changes: %s\n',cato);
                    end
                    if(max(max(diffs_metric(:,:,3))) >1E-3)
                        olix=find( max(diffs_metric(:,:,3),[],2) > 1E-3);
                        fprintf(fin,'\tIND (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f [%0.3f, %0.3f, %0.3f %0.3f], ',metric_names{i},median(diffs_metric(olix,i,3)),min(diffs_metric(olix,i,3)),prctile(diffs_metric(olix,i,3),25),prctile(diffs_metric(olix,i,3),75),max(diffs_metric(olix,i,3)) )]; end
                        fprintf(fin,'\t\tMedian [min, Q1, Q3, max] abs. difference, in runs showing changes: %s\n',cato);
                    end
                else
                ...
                if( Nvox1 == Nvox2 )
                    ...
                    if( max(diff_spms(:)) > 1E-3 ) %% any major differences in pipes
                        
                        fprintf(fin,'Comparing optimized SPMs..........................................[!!!]\n');
                        fprintf(fin,'\tDifferences in: \n');
                        if(max(max(diff_spms(:,:,1))) >1E-3)
                            olix=find( max(diff_spms(:,:,1),[],2) > 1E-3);
                            fprintf(fin,'\tCON (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                            cato=[];
                            for(i=1:size(diff_spms,2)) cato = [cato,sprintf( '(contrast-%u)=%0.3f [%0.3f, %0.3f, %0.3f, %0.3f], ',i,median(corr_spms(olix,i,1)),min(corr_spms(olix,i,1)),prctile(corr_spms(olix,i,1),25),prctile(corr_spms(olix,i,1),75),max(corr_spms(olix,i,1)) )]; end
                            fprintf(fin,'\t\tMedian [min, Q1, Q3, max] SPM corr., in runs showing changes: %s\n',cato);
                        end
                        if(max(max(diff_spms(:,:,2))) >1E-3)
                            olix=find( max(diff_spms(:,:,2),[],2) > 1E-3);
                            fprintf(fin,'\tFIX (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                            cato=[];
                            for(i=1:size(diff_spms,2)) cato = [cato,sprintf( '(contrast-%u)=%0.3f [%0.3f, %0.3f, %0.3f, %0.3f], ',i,median(corr_spms(olix,i,2)),min(corr_spms(olix,i,2)),prctile(corr_spms(olix,i,2),25),prctile(corr_spms(olix,i,2),75),max(corr_spms(olix,i,2)) )]; end
                            fprintf(fin,'\t\tMedian [min, Q1, Q3, max] SPM corr., in runs showing changes: %s\n',cato);
                        end
                        if(max(max(diff_spms(:,:,3))) >1E-3)
                            olix=find( max(diff_spms(:,:,3),[],2) > 1E-3);
                            fprintf(fin,'\tIND (%u of %u runs): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                            cato=[];
                            for(i=1:size(diff_spms,2)) cato = [cato,sprintf( '(contrast-%u)=%0.3f [%0.3f, %0.3f, %0.3f, %0.3f], ',i,median(corr_spms(olix,i,3)),min(corr_spms(olix,i,3)),prctile(corr_spms(olix,i,3),25),prctile(corr_spms(olix,i,3),75),max(corr_spms(olix,i,3)) )]; end
                            fprintf(fin,'\t\tMedian [min, Q1, Q3, max] SPM corr., in runs showing changes: %s\n',cato);
                        end
                    else
                ...
                elseif( abs( tiedrank(pipe_idx12(:,1))-tiedrank(pipe_idx12(:,2)) ) > TOLERANCE )
                    fprintf(fin,'Same set of tested steps?.........................................[X]\n');
                    fprintf(fin,'\tpermuted pipeline ordering, cannot do comparison - check your input files\n');
                elseif( max(abs(dout1.pipeset - dout2.pipeset)) > TOLERANCE )
                    fprintf(fin,'Same set of tested steps?.........................................[X]\n');
                    ...
                    if( max(diffx(:)) > TOLERANCE ) %% any major differences in pipes (subj x ppln x metric)
                        
                        fprintf(fin,'Comparing all pipeline metrics....................................[!!]\n');
                        fprintf(fin,'\tDifferences in: \n');
                        
                        olix=find( max(max(diffx,[],3),[],2) > TOLERANCE);
                        fprintf(fin,'\tRuns (%u of %u): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        olix=find( max(max(diffx,[],3),[],1) > TOLERANCE);
                        fprintf(fin,'\tPipelines (%u of %u)\n',length(olix),size(pipeset,1));
                        %
                        nonzero_diffs = diffx(diffx>TOLERANCE);
                        ...
                        for(t=1:size(pipeset,2))
                            ...
                            if( length(uniq)>1 )
                                ...
                                effecte(t,:) = (min(vll)==0) & (max(vll)>TOLERANCE);
                            else
                                effecte(t,1) = false;
                            end
                        end
                    ...
                    if( Nvox1 == Nvox2 )
                        ...
                        if( max(diffx(:)) > TOLERANCE ) %% any major differences in pipes (subj x ppln x contr)
                            ...
                            fprintf(fin,'\tPipelines (%u of %u)\n',length(olix),size(pipeset,1));
                            %
                            fprintf(fin,'\tMedian [min, max] abs. difference, in runs showing changes: %0.3f [%0.3f, %0.3f]\n',median(diffx(diffx>TOLERANCE)),min(diffx(diffx>TOLERANCE)),max(diffx(diffx>TOLERANCE)));
                ...
                if( prod(Nvox1==Nvox2) ) %% if all matching
                    ...
                    if( max(voldif_u(:)) > TOLERANCE ) %% any major differences in pipes (subj x ppln x contr)
                        
                        fprintf(fin,'Comparing all pipeline Procs(unnorm)..............................[!!]\n');
                        fprintf(fin,'\tDifferences in: \n');
                        olix=find( max(voldif_u,[],2) > TOLERANCE);
                        fprintf(fin,'\tRuns (%u of %u): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        olix=find( max(voldif_u,[],1) > TOLERANCE);
                        if(numel(olix)>10)
                        ...
                        fprintf(fin,'\tMedian [min, max] abs. difference, in runs showing changes: %0.3f [%0.3f, %0.3f]\n',median(voldif_u(voldif_u>TOLERANCE)),min(voldif_u(voldif_u>TOLERANCE)),max(voldif_u(voldif_u>TOLERANCE)));
                    else
                        fprintf(fin,'Comparing all pipeline Procs(unnorm)..............................[OK]\n');
                    end
                    if( max(voldif_s(:)) > TOLERANCE ) %% any major differences in pipes (subj x ppln x contr)
                        
                        fprintf(fin,'Comparing all pipeline Procs(spnorm)..............................[!!]\n');
                        fprintf(fin,'\tDifferences in: \n');
                        olix=find( max(voldif_s,[],2) > TOLERANCE);
                        fprintf(fin,'\tRuns (%u of %u): %s\n',length(olix),Nsubject,num2str(olix(:)','%u,'));
                        olix=find( max(voldif_s,[],1) > TOLERANCE);
                        if(numel(olix)>10)
                        ...
                        fprintf(fin,'\tMedian [min, max] abs. difference, in runs showing changes: %0.3f [%0.3f, %0.3f]\n',median(voldif_s(voldif_s>TOLERANCE)),min(voldif_s(voldif_s>TOLERANCE)),max(voldif_s(voldif_s>TOLERANCE)));
                    else
