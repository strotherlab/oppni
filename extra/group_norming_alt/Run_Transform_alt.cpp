...
// Updated 2013/07/19
//
// Run_Transform_alt: runs group-level spatial normalization across subjects
// based on anatomical template, for data generated by Pipeline_Step1.
//
// Call as:   
//
// "g++ -o Run_Transform_alt.out Run_Transform_alt.cpp"
//
// "./Run_Transform_alt.out {subject inputs}.txt {reference T1 anatomical}.nii {pipelines}.txt"
// 
...
int main(int argc, char* argv[])
{
    ...
    inpipelinename = argv[3]; // ARG#3: pipeline list file
...
// ----------------------- GROUP NORMALIZATION BEGINS -----------------------  //
                     ...
                     ss.str(std::string()); 
                     ss << "gunzip " << OUTstr_sub1 << "/*_sNorm.nii.gz";
                     s = ss.str();
                     returned = system( s.c_str() );                        
