function [p, h, stats] = signrank_octave(x,y,varargin)
...
if ~isempty(varargin)
    error('This version of signrank can only use the defult values of alpha = 0.05, method = [], and tail = both');
else
    disp('This version of signrank will use the defaults of alpha = 0.05, method = [], and tail = both');
    method = [];
    tail = 'both';
end
...
elseif strcmpi(method, 'oldexact')
    ...
    % This method can not be used since no settings are allowed
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	method = internal.stats.getParamVal(method,{'exact' 'approximate'},'''method''');
end

% Forcing Tail to be a two sided test
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Check the tail argument
%tail = internal.stats.getParamVal(tail,{'both' 'right' 'left'},'''tail''');
if ~strcmp(tail,'both' )
    error('This version of signrank can only use the defult values of tail = both');
else
    disp('This version of signrank will use the default of tail = both');
    tail = 'both';
end
