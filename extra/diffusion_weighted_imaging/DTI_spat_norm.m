function DTI_spat_norm(InputStruct,reference_file)
...
for ksub = 1:Nsubject
    ...
    if(InputStruct(ksub).run(1).MULTI_RUN_HARDI==0)
         dwi_ver = 'DTI';
    else dwi_ver = 'HARDI';
    end
    ...
    for(i=1:length(otherlist))
        ...
        innorm = [dti_path,'/',dwi_ver,'_fit_',otherlist{i}];
        outnorm = [dti_path,'/spat_norm/',dwi_ver,'_fit_',otherlist{i}];
        % second nonlinear transform, to final template (check if exist first)
