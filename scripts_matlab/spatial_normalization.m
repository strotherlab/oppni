...
if flag_step==0 || flag_step==2 || flag_step==3
    ...
    for ksub = 1:numel(InputStruct)
        ...
        for krun=1:numel(InputStruct(ksub).run)
            ...
            transform               = sprintf('%s/intermediate_processed/spat_norm/%s-affine/Transmat_EPItoREF%s.mat',InputStruct(ksub).run(krun).Output_nifti_file_path,reference_prefix,InputStruct(ksub).run(1).subjectprefix);
            transform_e2s           = sprintf('%s/intermediate_processed/spat_norm/%s-affine/Transmat_EPItoT1%s.mat',InputStruct(ksub).run(krun).Output_nifti_file_path,reference_prefix,InputStruct(ksub).run(1).subjectprefix);
