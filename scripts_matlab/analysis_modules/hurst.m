function output = hurst( datamat, split_info, Resampling_Index )
%
% =========================================================================
...
output.attributes.model_name  = 'hurst';
...
%% GROUP LEVEL ANALYSIS
    ...
    for(i=1:N_subject)
        ...
        if( (isfield(split_info{1},'hurst_units') && ~isempty(split_info{1}.hurst_units)) && (isfield(split_info{1},'hurst_range') && ~isempty(split_info{1}.hurst_range)) )
            %
            % if both units and range correctly specified
            [hurst_set(:,i)] = DFA_parallel( datamat{i}, split_info{1}.hurst_units, split_info{1}.hurst_range );
        else
            % otherwise default
            [hurst_set(:,i)] = DFA_parallel( datamat{i} );
        end
    end
    % remove non-finite computations
    hurst_set(~isfinite(hurst_set))=0.5 + eps;
    ...
    for k = 1:N_resample
        ...
        % split-half averages
        hurst_map1 = mean( hurst_set(:,set1),2);
        hurst_map2 = mean( hurst_set(:,set2),2);

        %% map manipulations for results

        % reweight the correlations, controlling vasc variance
        hurst_map1 = hurst_map1 .* split_info{1}.spat_weight;
        hurst_map2 = hurst_map2 .* split_info{1}.spat_weight;
        % average correlation map

        % reproducibility and SPM:
        [ R_allvox, rSPMZ_temp ] = get_rSPM( hurst_map1, hurst_map2, 1 );
    ...
    elseif( strcmp( split_info{1}.spm, 'raw'   ) ) output.images  =  mean(hurst_set,2);
