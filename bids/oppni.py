...
def run_part_one(bids_dir, subject_label, task_name, output_dir):
    ...
    list_bold_nii = glob(os.path.join(bids_dir, "sub-%s" % subject_label, "func", func_pattern))

    anat_pattern = "sub-{}_T1w.nii*".format(subject_label)
    anat_file_list = glob(os.path.join(bids_dir, "sub-%s" % subject_label, "anat", anat_pattern))

    task_json = os.path.join(bids_dir,'task-{}_bold.json'.format(task_name))
    events_tsv = os.path.join(bids_dir, "sub-%s_task-{}_events.tsv".format(subject_label,task_name))

    for idx, func_file in enumerate(list_bold_nii):
        # when multiple sessions exists:
        # glob(os.path.join(args.bids_dir, "sub-%s" % subject_label, "ses-*", "func", "*_bold.nii*")
        cmd = "run_oppni.sh %s PART1 %s %s %s %s %s %d %d %s %s" % (mcr_path, subject_label, func_file,
                                                                 output_dir, anat_file_list[idx],
