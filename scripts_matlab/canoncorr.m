...
if nargout > 5
    ...
    stats.pChisq = zeros(1,length(stats.chisq));
    for stat_counter = length(stats.chisq):-1:1
        try
            stats.pChisq(stat_counter) = 1 - chi2cdf(stats.chisq(stat_counter), stats.df1(stat_counter));
        catch
            stats.pChisq(stat_counter) = 1 - normcdf(stats.chisq(stat_counter), stats.df1(stat_counter),sqrt(2*stats.df1(stat_counter)));
        end
        if stats.pChisq(stat_counter)<signif_cutoff
            break;
        end
    end
