            ...
            if( isempty(metric_names) )
                ...
                if( max(diffs_metric(:)) > 1E-6 ) %% any major differences in pipes
                    ...
                    if(max(max(diffs_metric(:,:,1))) >1E-6) 
                        ...
                        cato=[];
                        for(i=1:length(metric_names)) cato = [cato,sprintf( '(%s)=%0.3f [%0.3f, %0.3f], ',metric_names{i},median(diffs_metric(olix,i,1)),min(diffs_metric(olix,i,1)),max(diffs_metric(olix,i,1)) )]; end
